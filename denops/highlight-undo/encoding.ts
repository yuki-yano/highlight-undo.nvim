...
export class EncodingUtil {
   ...
   */
  static getByteLength(text: string): number {
   ...
   */
  private static getCharDisplayWidth(char: string): number {
    ...
    if (this.isCombiningCharacter(code)) {
      return 0;
    }
   ...
   */
  private static isCombiningCharacter(code: number): boolean {
    return (
   ...
   */
  static splitIntoGraphemes(text: string): string[] {
    // Simple implementation - can be enhanced with Intl.Segmenter when available
    const graphemes: string[] = [];
    let current = "";

    for (const char of text) {
      if (current && this.isCombiningCharacter(char.charCodeAt(0))) {
        current += char;
      } else {
        if (current) {
   ...
   */
  static safeSlice(text: string, start: number, end?: number): string {
    const graphemes = this.splitIntoGraphemes(text);
